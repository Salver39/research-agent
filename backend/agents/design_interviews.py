...
from prompts.design_interviews import SYSTEM


class DesignInterviewsAgent(BaseAgent):
    ...
    def build_messages(self, state: dict[str, Any], user_input: str) -> list[dict]:
        ...
        if research_goal:
            goal_block = (
                f"\nЦель исследования (уже сформулирована на этапе диагноза, используй verbatim):\n{research_goal}\n"
            )
        if research_tasks:
            goal_block += (
                f"Задачи исследования (уже сформулированы, используй verbatim):\n"
        ...
        if out_of_scope:
            excluded_block = (
                "\nИСКЛЮЧЁННЫЕ гипотезы (этот метод не подходит для их проверки — НЕ создавай для них блоков, НЕ упоминай их в гайде):\n"
